...
// dropdown helpers (prevents UI errors)
...
document.addEventListener("click", (e) => {
  const dd = e.target.closest?.(".download-dropdown-wrap");
...
function buildCoverageFieldsFromDb(lobsById, lobFieldsRows) {
  ...
  for (const [lobId, secMap] of grouped.entries()) {
    ...
    for (const sectionName of sections) {
      ...
      for (const f of rows) arr.push({ id: String(f.id), label: f.label, isHeader: false });
    }
...
async function loadChecklistsFromDb() {
  ...
  if (joinErr) console.warn("checklist_lobs missing/blocked:", joinErr);
...
async function upsertChecklistToDb(cl) {
  ...
  if (lobIds.length) {
    ...
    if (jErr) console.warn("checklist_lobs insert blocked:", jErr);
  }
    ...
    .map((lobName) => {
      ...
      const rows = fields
        .map((f) => {
          ...
          const statusCls =
            { Match: "ss-match", "Not Match": "ss-notmatch", "Not Found": "ss-notfound", "N/A": "ss-na" }[e.status || "N/A"] ||
            "ss-na";

          const locked = e.status === "Match" || e.status === "N/A";
          const rowCls = locked ? "row-locked" : e.status === "Not Match" ? "row-notmatch" : "";
          ...
          return `<tr class="${rowCls}" id="row-${esc(f.id)}">
            ...
            <td style="width:18%"><input class="cell-input" value="${esc(
              e.pol || ""
            )}" oninput="updateEntry('${esc(f.id)}','pol',this.value)" placeholder="Policy data" ${ro}/></td>
            <td style="width:18%"><input class="cell-input" value="${esc(
              e.nex || ""
            )}" oninput="updateEntry('${esc(f.id)}','nex',this.value)" placeholder="Nexsure data" ${ro}/></td>
...
function updateStatusAndRow(fId, sel) {
  ...
  if (row) {
    ...
    if (sel.value === "Match" || sel.value === "N/A") row.classList.add("row-locked");
    if (sel.value === "Not Match") row.classList.add("row-notmatch");
  }
    ...
    html += `<div class="snapshot-card">
      ...
      <div class="snap-grid">
        <div><div class="snap-label">Policy Document</div>
          <div class="snap-upload-area"><div style="font-size:30px;opacity:.3">🖼️</div><div style="font-size:12px;font-weight:600">Upload later</div></div>
        </div>
...
function renderAdminUsers() {
  ...
  body.innerHTML = `<tr><td colspan="6" style="padding:14px;color:var(--muted);font-size:13px">
    Use “＋ Add User” to create users (Admin only). If it errors, your Vercel API is misconfigured.
  </td></tr>`;
}

// FIXED: Add LOB works with correct IDs
async function addNewLOB() {
  try {
    const name = document.getElementById("new-lob-name")?.value?.trim();
    const code = document.getElementById("new-lob-code")?.value?.trim()?.toUpperCase() || null;

    if (!name) return showToast("⚠ Enter LOB name");

    const { error } = await supabase.from("lobs").insert([{ name, code, is_active: true }]);
...
// FIXED: Save User uses your Vercel API
async function saveUser() {
  if (!isAdmin()) return showToast("⛔ Admin only");

  const username = document.getElementById("uf-username")?.value?.trim() || null;
  ...
  try {
    ...
    if (!token) return showToast("⚠ No session token. Login again.");
    ...
    if (!resp.ok) return showToast("❌ " + (json.error || "Create user failed"));
    ...
    console.error(e);
    showToast("❌ Create user error");
  }
}

// ─────────────────────────────────────────────────────────────
// PREVIEW / DOWNLOAD placeholders (prevents UI errors)
// ─────────────────────────────────────────────────────────────
function showPreview(_mode) {
  showToast("Preview is coming next (UI wired, generator pending)");
}
function downloadAs(_mode) {
  showToast("Download PDF is coming next (generator pending)");
}
